...
<!--              </el-date-picker>-->
        ...
        <el-form-item label="客服类型" label-width="80px">
          <el-select
            filterable
        ...
        <el-form-item style="margin-left:10px">
          <el-button @click="setCsCompositeScoreList">批量导入综合得分</el-button>
        </el-form-item>
          <!-- <el-form-item style="margin-left:10px">
      ...
      <!-- 加减量 表格区 -->
      <el-table :cell-style="{padding:'0'}" :data="csData" :header-cell-class-name="'table-header'" :row-class-name="tableRowClassName" border height="450" size="small" style="width: 100%" v-loading="loading">
        <el-table-column prop="csName" label="客服名" align="center" width="150">
          <template slot-scope="scope">
            {{scope.row.agentName ? scope.row.agentName : scope.row.agentId === null ? "总计" : scope.row.agentId}}
        ...
        <el-table-column prop="severAccidentCount" label="重大事故" align="center">
          <template slot-scope="scope">
            {{getValue(scope.row.severAccidentCount, 'Num')}}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="plusAndMinusScoreTotal" label="加减分得分" align="center" width="100">
          <template slot-scope="scope">
            {{getValue(scope.row.plusAndMinusScoreTotal, 'NumWithZero')}}
          </template>
        </el-table-column>
        <el-table-column prop="compositeScore" label="综合得分" align="center">
          <template slot-scope="scope">
            {{getValue(scope.row.compositeScore, 'NumWithZero')}}
          </template>
        </el-table-column>
        <el-table-column prop="scoreTotal" label="电话总得分" align="center" width="100">
          <template slot-scope="scope">
    ...
    <el-dialog :title="'备注'" :visible.sync="remarkDialog" width="35%" @close="closeDialog">
      <span>
        <el-form :model="remarkObj" ref="addFormRef">
          <el-row>
            <el-col :span="24">
              <el-form-item label="备注" prop="remark" label-width="50px">
                ...
                <span>{{ remarkObj.remark ? remarkObj.remark.length : 0 }}  / 30</span>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
    ...
    <el-dialog class="score-dialog" :title="'设置客服综合得分'" :visible.sync="compositeDialog" width="45%" @close="closeCompositeScoreDialog">
      <span class="queryBar">
        <el-form calss="form-query" inline ref="form" size="small" :model="compositeQueryMap" >
          <el-form-item class="form-item" label="部门" prop="department"  label-width="60px">
            <el-select
              filterable
              v-model="compositeQueryMap.department"
              placeholder="请选择"
              class="select"
              @change="changeCompositeQuery">
              <el-option
                v-for="(dept, i) in dialogDepartmentList"
                :label="dept"
                :key="i"
                :value="dept">
              </el-option>
          ...
          <el-form-item label="客服类型" label-width="80px">
            <el-select
              filterable
              v-model="compositeQueryMap.csType"
              @change="changeCompositeQuery"
              placeholder="请选择">
                <el-option
                  v-for="type in csTypeList"
      ...
      <span>
        <el-table :data="csCompositeData" 
          style="width: 70%" size="small" 
          :cell-style="{padding:'0'}" 
          :header-cell-class-name="'table-header'" 
          border 
          v-loading="csCompositeLoading" 
          ...
          <el-table-column prop="compositeScore" label="综合得分" align="center">
            <template slot-scope="scope">
              <el-input-number v-model="scope.row.compositeScore" :controls="false" :precision="2" :min="0" :max="10"></el-input-number>
            </template>
          </el-table-column>
        </el-table>
      </span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="closeCompositeScoreDialog">取 消</el-button>
        <el-button
          type="primary" 
          :loading="btnLoading"
          :disabled="btnDisabled"
...
export default {
  data() {
    return {
      ...
      loading: false,
      csCompositeLoading: false
    }
  },
  computed: {
    //  客服类型清单
    csTypeList() {
      let list = cloneDeep(constants.CUSTOMER_SERVICE_TYPE_LIST)
      list = list.filter(i => i.id === 12 || i.id === 13)
      return list
    }
  },
  methods: {
     ...
     */
    async transferProductChange(scope) {
      ...
      let params = {
        ...
        csType: scope === "composite" ? this.compositeQueryMap.csType : this.queryMap.csType
      }
      const { data: res } = await this.$http.get("system/agentServiceSound/getCs", { params });
      if(!res.success) {
        this.$message.error("获取客服数据失败:" + res.data.errorMsg);
      } else {
        scope !== "composite" && (this.csList = res.data)
        scope === "composite" && (this.compositeCsList = res.data)
    ...
    downExcel() {
      ...
      const $this = this;
      axios.request({
        url: "system/phoneCsReport/getSalesCsScoreReport/export",
        method: "get",
        responseType: "blob",
        params: params
      })
    ...
    async searchData() {
      ...
      let { data: res } = await self.$http.get(
        'system/phoneCsReport/getSalesCsScoreReport', {params}
      );
      ...
      if (res.success) {
        self.checkQueryMap = cloneDeep(self.queryMap)
    ...
    // 开启综合得分弹窗
    setCsCompositeScoreList() {
      this.compositeQueryMap.department = cloneDeep(this.queryMap.department)
      this.compositeQueryMap.csType = cloneDeep(this.queryMap.csType)
      this.dialogDepartmentList = cloneDeep(this.departmentList)
      this.changeCompositeQuery()
      this.compositeDialog = true
    },
    // 关闭综合得分弹窗
    closeCompositeScoreDialog() {
      this.compositeDialog = false
      this.csCompositeData = []
      this.compositeCsList = []
    },
    // 更新综合得分
    async handleUpdateScore() {
      let params = {
        csReportScoreConfigVoList: this.csCompositeData
      }
      this.btnLoading = true
      let { data: res } = await this.$http.post('system/csReportScoreConfig', params);
      if(res.success) {
        this.$notify.success({
          title:'操作成功',
        });
        this.queryMap.department = cloneDeep(this.compositeQueryMap.department)
        this.queryMap.csType = cloneDeep(this.compositeQueryMap.csType)
        this.searchData()
        this.closeCompositeScoreDialog()
      } else {
    ...
    async saveOrUpdateRemark() {
      this.btnLoading = true
      let { data: res } = await this.$http.post('system/reportRemark', this.remarkObj);
      if(res.success) {
        this.$notify.success({
            title:'操作成功',
        });
    ...
    // 综合得分弹窗切换部门或客服类型事件
    async changeCompositeQuery() {
      if(null == this.compositeQueryMap.department || '' === this.compositeQueryMap.department) {
        return;
      }
      (this.compositeQueryMap.csType !== this.checkCompositeQueryMap.csType) && this.transferProductChange("composite");

      this.csCompositeLoading = true
      let params = {
        department: this.compositeQueryMap.department,
        ...
        csType: this.compositeQueryMap.csType
      }
      const { data: res } = await this.$http.get("system/csReportScoreConfig", {params});
      if(!res.success) {
        this.$message.error({
          message: res.data.errorMsg,
          type: 'error',
        ...
        this.csCompositeData = cloneDeep(res.data)
        this.checkCompositeQueryMap = cloneDeep(this.compositeQueryMap)
      }
      this.csCompositeLoading = false
    }
  },
  created() {
    this.defaultDate()
